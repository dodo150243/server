...
const myskinre = {
  getAllOrderDone: function (callback) {
    return db.query(
      `SELECT id_order, COUNT(id_task) AS items 
      FROM control_doe 
      WHERE id_task 
      GROUP BY id_order 
      HAVING COUNT(id_task) = COUNT(done_on) 
  ...
  getAllOrderNotDone: function (callback) {
    return db.query(
      `SELECT id_order,COUNT(id_task) AS items,COUNT(done_on) AS done, COUNT(id_task)-COUNT(done_on) as deficit 
      FROM control_doe 
      WHERE id_task 
      GROUP BY id_order 
      HAVING COUNT(id_task)-COUNT(done_on) 
  ...
  updateTask: function (id, data, callback) {
    return db.query(
      `UPDATE control_doe SET done_on=? 
      WHERE id_task=?`,
      [data.done_on, id],
      callback,
      console.log(data)
